    ...
    const patterns = [
      ...
      },
      {
        ...
        render: (m, k) => (
          <code
            key={k}
            className="px-1 py-0.5 rounded bg-white/10 text-green-300 text-[11px] font-mono"
          >
            {m[1]}
          </code>
        ),
    ...
    while (txt.length > 0) {
      ...
      for (const pat of patterns) {
        ...
        if (m && (!best || m.index < best.m.index))
          best = { m, render: pat.render };
      }
      if (!best) {
        result.push(<span key={key++}>{txt}</span>);
        break;
      }
      if (best.m.index > 0)
        result.push(<span key={key++}>{txt.slice(0, best.m.index)}</span>);
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith("```")) {
      ...
      elements.push(
        <pre
          key={`cb-${i}`}
          className="my-2 p-3 rounded-xl overflow-x-auto text-left"
          style={{
        ...
        >
          {lang && (
            <div className="text-[9px] text-green-400/60 uppercase tracking-wider mb-1.5 font-mono">
              {lang}
            </div>
          )}
          <code className="text-[11px] text-green-300 font-mono whitespace-pre">
            {codeLines.join("\n")}
          </code>
        </pre>,
      );
    ...
    if (line.startsWith("# ")) {
      elements.push(
        <h1 key={`h1-${i}`} className="text-sm font-bold text-white mt-3 mb-1">
          {processInline(line.slice(2), i * 100)}
        </h1>,
      );
      i++;
      continue;
      ...
      elements.push(
        <div key={`h2-${i}`} className="flex items-center gap-2 mt-3 mb-1.5">
          ...
          <h2 className="text-sm font-bold text-white">
            {processInline(line.slice(3), i * 100)}
          </h2>
        </div>,
      );
      i++;
    ...
    if (line.startsWith("### ")) {
      elements.push(
        <h3
          key={`h3-${i}`}
          className="text-xs font-semibold text-green-300/90 mt-2 mb-1 uppercase tracking-wide"
        >
          {processInline(line.slice(4), i * 100)}
        </h3>,
      );
      i++;
      continue;
        ...
        <ol key={`ol-${i}`} className="space-y-1.5 my-1.5 ml-1">
          {items.map((item, idx) => (
            <li key={idx} className="flex items-start gap-2.5">
              <span
                className="w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold text-green-300 flex-shrink-0 mt-0.5"
                style={{
    ...
    elements.push(
      <p key={`p-${i}`} className="text-sm text-white/85 leading-relaxed">
        {processInline(line, i * 100)}
      </p>,
    );
    i++;
  }
...
const AskVeAg = ({ isOpen, onClose, caseId, userId, diseaseName, t }) => {
  ...
  useEffect(() => {
    ...
    const hasPending = messages.some(
      (m) =>
        (m.status === "analyzing" || m.status === "sent") &&
        !m.reply &&
  ...
  const loadMessages = async (before = null, silent = false) => {
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/ask/${caseId}/messages`, {
        params,
      });
  ...
  const validateInput = (text) => {
    ...
    if (
      /<[^>]+>/.test(text) ||
      /data:(image|application|video|audio)\//.test(text)
    ) {
      showError(
        t.caseDetail?.askVeAgTextOnly || "Only text messages are allowed",
      );
      return false;
    }
    return true;
  ...
  const handleRetry = async (messageId, originalText = null) => {
    ...
    if (messageId.startsWith("temp-")) {
      ...
      try {
        ...
        if (res.data.success) {
          setMessages((prev) =>
            prev.map((m) => (m._id === newTempId ? res.data.message : m)),
          );
        } else {
          setMessages((prev) =>
            prev.map((m) =>
              m._id === newTempId ? { ...m, status: "failed" } : m,
            ),
          );
          showError(
            res.data.error ||
    ...
    setMessages((prev) =>
      prev.map((m) =>
        m._id === messageId ? { ...m, status: "analyzing" } : m,
      ),
    );

    try {
      const res = await axios.post(
        `${API_BASE_URL}/ask/messages/${messageId}/retry`,
        { userId },
      );
      if (res.data.success) {
        setMessages((prev) =>
      ...
      setMessages((prev) =>
        prev.map((m) => (m._id === messageId ? { ...m, status: "failed" } : m)),
      );
      showError(t.caseDetail?.askVeAgError || "Something went wrong");
    } finally {
      setSending(false);
  ...
  const systemMessages = [
    {
      id: "sys-1",
      text: t.caseDetail?.askVeAgWelcome || "Welcome to Ask VeAg!",
    },
    {
      id: "sys-2",
      text: (
        t.caseDetail?.askVeAgWelcomeMsg ||
        "I'm your AI assistant specialized in helping with the disease detected in your crop."
      ).replace("{disease}", diseaseName),
    },
    {
      id: "sys-3",
      text:
        t.caseDetail?.askVeAgRules ||
        "Important: I can only answer questions related to the detected disease. Text messages only.",
    },
    {
      id: "sys-4",
      text:
        t.caseDetail?.askVeAgPrivacy ||
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(34,197,94,0.12) 0%, rgba(0,0,0,0.45) 50%, rgba(34,197,94,0.08) 100%)",
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-bold text-base leading-tight">
                    {t.caseDetail?.askVeAgTitle || "Ask VeAg"}
                  </h3>
                  <p className="text-green-300/70 text-xs">
                    {t.caseDetail?.askVeAgSubtitle ||
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-xs text-green-300/70">
                      {t.caseDetail?.askVeAgLoadingOlder ||
            ...
            >
              ...
              {systemMessages.map((sys) => (
                <div key={sys.id} className="flex justify-center my-2">
                  <div
                    className="max-w-[85%] px-4 py-2.5 rounded-2xl text-center"
                    style={{
                      background:
              ...
              {groupedMessages.map((item) => {
                ...
                return (
                  <div key={item.key}>
                    ...
                    <div className="flex justify-end mb-1">
                      <div className="max-w-[80%] group relative">
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg, rgba(34,197,94,0.35), rgba(22,163,74,0.3))",
                            border: "1px solid rgba(34,197,94,0.25)",
                          }}
                        >
                        ...
                        <div className="absolute -left-8 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-1">
                          <button
                            ...
                            onClick={() =>
                              handleCopy(msg.message, `user-${msg._id}`)
                            }
                            className="p-1 rounded-full bg-white/10 hover:bg-white/20 transition-colors"
                            title={t.caseDetail?.askVeAgCopyMsg || "Copy"}
                          >
                            {copiedId === `user-${msg._id}` ? (
                        ...
                        {msg.status === "failed" && (
                          <div className="flex items-center justify-end gap-1 mt-0.5">
                            ...
                            <span className="text-[10px] text-red-400">
                              {t.caseDetail?.askVeAgFailed || "Failed to send"}
                            </span>
                            <button
                              onMouseDown={(e) => e.preventDefault()}
                              onClick={() => handleRetry(msg._id, msg.message)}
                    ...
                    {/* AI Reply */}
                    {(msg.status === "sending" || msg.status === "analyzing") &&
                      !msg.reply && (
                        ...
                        <div className="max-w-[85%] group relative">
                          ...
                          >
                            ...
                            <div className="flex items-center justify-end mt-1.5">
                              <span className="text-[10px] text-white/25">
                                {getTimeLabel(msg.updatedAt || msg.createdAt)}
                              </span>
                            </div>
                          </div>
                          {/* Copy for AI reply */}
                          <div className="absolute -right-8 top-4 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                              onMouseDown={(e) => e.preventDefault()}
                              onClick={() =>
                                handleCopy(msg.reply, `ai-${msg._id}`)
                              }
                              className="p-1 rounded-full bg-white/10 hover:bg-white/20 transition-colors"
                              title={t.caseDetail?.askVeAgCopyMsg || "Copy"}
                            >
            ...
            <div className="px-4 py-3 border-t border-white/10">
              <div className="flex items-end gap-2">
                ...
                <div className="flex-1 relative">
                  <textarea
                    ...
                    placeholder={
                      t.caseDetail?.askVeAgPlaceholder ||
                ...
                <button
                  onMouseDown={(e) => {
                    e.preventDefault();
                    if (!sending && !isAiProcessing && input.trim())
                      handleSend();
                  }}
              ...
              {input.length > 1800 && (
                <p className="text-[10px] text-orange-400/60 mt-1 text-right">
                  {input.length}/2000
                </p>
              )}
            </div>

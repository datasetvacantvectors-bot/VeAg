...
const CaseDetail = ({ daysRemaining }) => {
  ...
  const fetchCaseDetail = async (silent = false, isRefreshBtn = false) => {
    ...
    try {
      ...
      if (response.data.case.userId !== currentUser.userId) {
        ...
        setSelectedImage(
          (prev) => prev || response.data.case.images[0]?.url || null,
        );
        ...
        if (response.data.case.status === "completed") {
          try {
            const resultResponse = await axios.get(
              `${API_BASE_URL}/cases/${caseId}/result`,
            );
    ...
    const poll = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_BASE_URL}/cases/${caseId}/treatment-info`,
        );
          ...
          remaining.forEach((type) => {
            if (response.data.treatments[type]) {
              setTreatmentData((prev) => ({
                ...prev,
                [type]: response.data.treatments[type],
              }));
      ...
      if (attempts >= MAX_ATTEMPTS) {
        remaining.forEach((type) => {
          ...
          setTreatmentError((prev) => ({
            ...prev,
            [type]: "Generation timed out. Please retry.",
          }));
  ...
  const generateTreatment = async (type) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/cases/${caseId}/treatment-info/${type}`,
      );
      if (response.data.success) {
        setTreatmentData((prev) => ({
          ...prev,
          [type]: response.data.treatmentInfo,
        }));
        saveTreatmentState(type, "completed");
      }
    } catch (err) {
      setTreatmentError((prev) => ({
        ...
        [type]:
          ...
          `Failed to generate ${type} information. Please try again.`,
      }));
      saveTreatmentState(type, "failed");
    } finally {
      setTreatmentLoading((prev) => ({ ...prev, [type]: false }));
        ...
        if (listType === "ordered") {
          elements.push(
            <ol
              ...
              className="list-decimal list-inside space-y-1.5 ml-2 mb-3"
            >
              {listItems.map((item, i) => (
                <li key={i} className="text-white/90 text-sm leading-relaxed">
                  {renderInline(item)}
                </li>
              ))}
            </ol>,
          ...
          elements.push(
            <ul key={`ul-${elements.length}`} className="space-y-1.5 ml-2 mb-3">
              {listItems.map((item, i) => (
                <li
                  key={i}
                  className="text-white/90 text-sm leading-relaxed flex items-start gap-2"
                >
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith("## ")) {
        ...
        elements.push(
          <h3
            key={`h-${i}`}
            className="text-base font-bold text-violet-300 mt-4 mb-2 flex items-center gap-2"
          >
    ...
    text.split("\n").forEach((line) => {
      ...
      if (trimmed.startsWith("## ")) {
        ...
        if (numMatch) {
          lines.push({
            type: "numbered",
            text: `${numMatch[1]}. ${numMatch[2].replace(/\*\*(.*?)\*\*/g, "$1")}`,
          });
        } else {
          lines.push({
            type: "text",
    ...
    try {
      ...
      const checkPageBreak = (needed = 20) => {
        if (y + needed > pageH - 18) {
          ...
          doc.text(`Case #${caseData.caseId}`, pageW - mr, 7, {
            align: "right",
          });
          y = 18;
        }
      ...
      doc.setTextColor(100, 116, 139);
      doc.text(
        `Generated: ${formatDate(caseResult.processedAt)}`,
        rx + 7,
        y + 24,
      );
      ...
      doc.setTextColor(180, 83, 9);
      doc.text(
        `${(caseResult.processingTime / 1000).toFixed(2)}s`,
        rightColX,
        y + 14,
      );
      doc.setFontSize(7);
      ...
      doc.setTextColor(180, 83, 9);
      doc.text(
        caseData.cropName.charAt(0).toUpperCase() + caseData.cropName.slice(1),
        rightColX,
        y + 25,
      );
      ...
      renderSection(
        "Treatment Guide",
        "Rx",
        treatmentData.treatment.content,
        {
          headerBg: [22, 163, 74],
        ...
        treatmentData.treatment.generatedAt ||
          treatmentData.treatment.createdAt,
      );
      renderSection(
        "Disease Causes",
        "!",
        treatmentData.causes.content,
        {
          headerBg: [217, 119, 6],
          headerText: [255, 255, 255],
          h2Color: [154, 78, 3],
          bulletDot: [245, 158, 11],
        },
      ...
      renderSection(
        ...
        "+",
        treatmentData.prevention.content,
        {
          headerBg: [37, 99, 235],
          headerText: [255, 255, 255],
          h2Color: [23, 64, 178],
          bulletDot: [59, 130, 246],
        },
        treatmentData.prevention.generatedAt ||
          treatmentData.prevention.createdAt,
      );
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(
          "This report is generated by VeAg AI platform. For agricultural advisory purposes only.",
          ml,
          ph - 5,
        );
        doc.text(`Page ${i} of ${totalPages}`, pageW - mr, ph - 5, {
          align: "right",
        });
      }
  ...
  const handleProcessCase = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/cases/${caseId}/process`,
      );
      ...
      setProcessingError(
        err.response?.data?.error || "Failed to start processing",
      );
    } finally {
      setProcessing(false);
    }
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      pending: {
        bg: "bg-yellow-600/80 border-yellow-400/50",
        text: "text-white",
        label: t.status.pending,
      },
      processing: {
        bg: "bg-blue-600/80 border-blue-400/50",
        text: "text-white",
        label: t.status.processing,
      },
      completed: {
        bg: "bg-green-600/80 border-green-400/50",
        text: "text-white",
        label: t.status.completed,
      },
      failed: {
        bg: "bg-red-600/80 border-red-400/50",
        text: "text-white",
    ...
    return (
      <span
        className={`px-4 py-2 rounded-full text-sm font-semibold border backdrop-blur-xl ${config.bg} ${config.text}`}
      >
        {config.label}
      </span>
    );
          ...
          <div className="flex flex-col items-center gap-6">
            ...
            <div>
              <h2 className="text-3xl font-bold text-white mb-2">
                {t.caseDetail.unauthorizedTitle}
              </h2>
              <p className="text-white/80 text-lg mb-6">
                {t.caseDetail.unauthorizedText ||
          ...
          <div className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl shadow-2xl p-4 sm:p-6 mb-6">
            <div className="flex flex-col lg:flex-row justify-between items-center lg:items-center gap-4 lg:gap-6 text-center lg:text-left">
              <div className="min-w-0 w-full lg:max-w-[60%]">
                ...
                <p className="text-sm sm:text-base text-white/70 break-words">
              ...
              <div className="flex w-full lg:w-auto flex-col sm:flex-row gap-3 items-stretch sm:items-center lg:justify-end">
                ...
                >
                  ...
                  />
                  {isRefreshing
                    ? t.caseDetail.refreshing || "Refreshing..."
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
                  ...
                  >
                    {processing ? (
                      ...
                      <>
                        ...
                        <span>
                          {caseData.status === "failed"
                            ? t.caseDetail.retryProcessing
                            : t.caseDetail.processAI}
                        </span>
                      </>
                    )}
                  </button>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h4 className="font-bold text-white">
                        {t.caseDetail.processingTitle}
                      </h4>
                      <p className="text-sm text-white/80">
                        {t.caseDetail.aiAnalyzing}
                      </p>
                    </div>
                  </div>
              ...
              {caseData.status === "completed" && caseResult && (
                <div
                  className={`mt-6 border-2 backdrop-blur-2xl rounded-lg p-6 ${
                    caseResult.diseaseStatus.toLowerCase() ===
                    "healthy rice leaf"
                ...
                >
                  <div className="flex items-center gap-3 mb-4">
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-xl border ${
                        caseResult.diseaseStatus.toLowerCase() ===
                        "healthy rice leaf"
                    ...
                    <div>
                      <h4 className="font-bold text-white text-lg">
                        {t.caseDetail.analysisComplete}
                      </h4>
                      <p className="text-sm text-white/80">
                        {t.caseDetail.aiResults}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white/10 backdrop-blur-xl rounded-lg p-4 mb-4 border border-white/30">
                    <h5 className="text-sm font-semibold text-white/70 mb-2">
                      {t.caseDetail.diseaseDetected}:
                    </h5>
                    <p className="text-xl font-bold text-white">
                      {caseResult.diseaseStatus}
                    </p>
                  </div>

                  <div className="text-xs text-white/70 space-y-1">
                    <p>
                      ⏱️ {t.caseDetail.processingTime}:{" "}
                      {(caseResult.processingTime / 1000).toFixed(2)}s
                    </p>
                    <p>
                      📅 {t.caseDetail.completed}:{" "}
                      {formatDate(caseResult.processedAt)}
                    </p>
                  </div>
                </div>
              )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-xl font-bold text-white">
                            {t.caseDetail.treatmentInsights ||
                              "Treatment & Insights"}
                          </h3>
                          <p className="text-sm text-white/70">
                            {t.caseDetail.treatmentInsightsDesc ||
                              "AI-powered treatment recommendations for"}{" "}
                            <span className="text-orange-300 font-semibold">
                              {caseResult.diseaseStatus}
                            </span>
                          </p>
                          <p className="text-sm text-white/70">
                            {t.caseDetail.treatmentInsightsInformation ||
                            ...
                            <div className="bg-gradient-to-br from-green-600/10 to-emerald-600/10 border border-green-400/30 rounded-xl p-5 backdrop-blur-xl">
                              ...
                              {treatmentLoading.treatment ? (
                                <div className="flex flex-col items-center gap-3 py-8">
                                  ...
                                  <p className="text-sm text-white/80">
                                    {t.caseDetail.generatingTreatment ||
                            ...
                            <div className="bg-gradient-to-br from-amber-600/10 to-orange-600/10 border border-amber-400/30 rounded-xl p-5 backdrop-blur-xl">
                              ...
                              {treatmentLoading.causes ? (
                                <div className="flex flex-col items-center gap-3 py-8">
                                  ...
                                  <p className="text-sm text-white/80">
                                    {t.caseDetail.generatingCauses ||
                                      "Analyzing disease causes..."}
                                  </p>
                                </div>
                              ) : treatmentError.causes ? (
                                <div className="bg-red-600/20 border border-red-400/40 rounded-lg p-4">
                                ...
                                <div>
                                  ...
                                  {(treatmentData.causes.generatedAt ||
                                    treatmentData.causes.createdAt) && (
                            ...
                            <div className="bg-gradient-to-br from-blue-600/10 to-cyan-600/10 border border-blue-400/30 rounded-xl p-5 backdrop-blur-xl">
                              ...
                              {treatmentLoading.prevention ? (
                                <div className="flex flex-col items-center gap-3 py-8">
                                  ...
                                  <p className="text-sm text-white/80">
                                    {t.caseDetail.generatingPrevention ||
                        ...
                        >
                          {reportGenerating ? (
                            <>
                              ...
                              />
                              <span>
                                {t.generateReport.generatingReport ||
                                  "Generating Report..."}
                              </span>
                            </>
                          ) : (
                            <>
                              ...
                              <span>
                                {t.generateReport.saveReport || "Save Report"}
                              </span>
                              {allSectionsFetched && (
              ...
              {caseData.status === "failed" && (
                <div className="mt-6 bg-red-600/20 border-2 border-red-400/50 backdrop-blur-2xl rounded-lg p-6">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      <h4 className="font-bold text-white">
                        {t.caseDetail.processingFailed}
                      </h4>
                      <p className="text-sm text-white/80">
                        {t.caseDetail.errorOccurred}
                      </p>
                    </div>
                  </div>
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl shadow-2xl p-6">
                <h3 className="text-xl font-bold text-white mb-4">
                  {t.caseDetail.caseStats}
                </h3>
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-white/20">
                    <span className="text-white/70">
                      {t.caseDetail.totalImages}
                    </span>

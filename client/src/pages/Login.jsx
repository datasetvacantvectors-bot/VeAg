...
const Login = () => {
  ...
  useEffect(() => {
    const updateConstraints = () => {
      if (trackRef.current) {
        setDragConstraints({
          left: 0,
          right: trackRef.current.offsetWidth - 56,
        }); // 48px handle + 8px padding
      }
    };
    updateConstraints();
  ...
  const renderModalContent = () => {
    switch (activeModal) {
      ...
      case "terms":
        return <TermsAndConditions isModal={true} />;
      case "refund":
        return <ReturnRefundCancellation isModal={true} />;
      case "shipping":
        return <ShippingAndDelivery isModal={true} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-300 via-orange-200 to-yellow-100 overflow-hidden relative flex items-center justify-center">
      {/* Animated Sky */}
      <motion.div
        className="absolute top-12 left-12 w-48 h-24 bg-white rounded-full opacity-60 blur-2xl"
        animate={{ x: [0, 50, 0] }}
        transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute top-32 right-20 w-64 h-32 bg-white rounded-full opacity-50 blur-3xl"
        animate={{ x: [0, -60, 0] }}
      ...
      {/* Sun glow effect */}
      <motion.div
        className="absolute top-1/4 left-1/2 -translate-x-1/2 w-96 h-96 bg-yellow-300 rounded-full opacity-30 blur-[100px]"
        ...
        <div className="w-full min-h-[100dvh] sm:min-h-0 sm:h-auto bg-white/40 backdrop-blur-3xl sm:backdrop-blur-2xl border-none sm:border sm:border-white/50 shadow-none sm:shadow-[0_8px_32px_0_rgba(0,0,0,0.1)] rounded-none sm:rounded-[2.5rem] overflow-hidden relative flex flex-col justify-center">
          ...
          <div className="p-6 sm:p-10 lg:p-12 flex flex-col items-center h-full sm:h-auto justify-center sm:justify-start">
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight text-center mb-3 flex flex-wrap justify-center items-baseline gap-x-2">
              ...
              <span className="text-white drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] font-extrabold py-1">
                VeAg
              </span>
            </h2>
              ...
              {loading ? (
                <div className="flex items-center gap-3">
                  <div className="relative w-5 h-5">
                    ...
                    />
                    <motion.div
                      className="absolute inset-1 rounded-full border-2 border-transparent border-b-green-500 border-l-green-500"
                      animate={{ rotate: -360 }}
                      transition={{
                        duration: 1.5,
                        repeat: Infinity,
            ...
            <div
              className="sm:hidden w-full relative h-14 bg-white/20 rounded-full border border-white/40 shadow-inner overflow-hidden flex items-center"
              ref={trackRef}
            >
              <motion.div
              ...
              <div className="absolute w-full text-center pointer-events-none z-0 pl-10 pr-4">
                <span
                  className={`text-sm font-bold transition-colors duration-300 ${agreed ? "text-gray-900 drop-shadow-sm" : "text-gray-400"}`}
                >
              ...
              >
                {loading ? (
                  ...
                  <svg
                    className={`w-5 h-5 transition-opacity ${agreed ? "opacity-100" : "opacity-50 grayscale"}`}
                    viewBox="0 0 24 24"
                    fill="currentColor"
                  >
                    <path
            ...
            <div className="w-full mt-8 pt-6 border-t border-gray-300 flex flex-wrap justify-center items-center gap-x-3 gap-y-2 text-[0.7rem] sm:text-xs text-gray-700 font-bold">
              <button
                ...
                className="hover:text-gray-900 transition-colors underline decoration-gray-400 underline-offset-2"
              >
                Terms
              </button>
              <span>•</span>

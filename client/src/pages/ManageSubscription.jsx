...
function ManageSubscription() {
  ...
  const fetchTransactionHistory = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/subscriptions/${currentUser.userId}/transactions`,
      );
  ...
  const fetchSubscriptionHistory = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/subscriptions/${currentUser.userId}/history`,
      );
  ...
  const handlePurchase = async () => {
    if (selectedMonths < PLAN.minMonths || selectedMonths > PLAN.maxMonths) {
      alert(
        `Please select between ${PLAN.minMonths} to ${PLAN.maxMonths} months`,
      );
      return;
    }
    ...
    try {
      ...
      const orderResponse = await axios.post(
        `${API_BASE_URL}/subscriptions/create-order`,
        {
      ...
      const options = {
        ...
        modal: {
          ondismiss: async () => {
            ...
            try {
              await axios.post(
                `${API_BASE_URL}/subscriptions/payment-failure`,
                {
      ...
      setPaymentNotification({
        ...
        details: {
          error:
            ...
            t.manageSubscription.errorStartPayment,
  ...
  const currentPlanHistory = subscriptionHistory.slice(
    indexOfFirstPlan,
    indexOfLastPlan,
  );
  const totalPlanPages = Math.ceil(
    subscriptionHistory.length / planHistoryPerPage,
  );
      ...
      <div className="min-h-screen bg-gradient-to-b from-orange-300 via-orange-200 to-yellow-100 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="mt-6 text-white text-lg font-semibold">
            {t.manageSubscription.loadingSubscription}
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-300 via-orange-200 to-yellow-100 relative overflow-hidden">
      ...
      {showSupport && (
        ...
        >
          ...
          <p className="text-white/90 mb-4">{t.manageSubscription.helpText}</p>
          <a
        ...
        >
          ...
          >
            ...
            <div className="space-y-3 mb-6">
                  ...
                  <>
                    <div className="bg-white/10 backdrop-blur-xl rounded-lg p-4 border border-white/20">
                      <p className="text-white/70 text-sm mb-1">
                        {t.manageSubscription.paymentId}
                      </p>
                      <p className="text-white font-mono text-xs break-all">
                        {paymentNotification.details.paymentId}
                      </p>
                    </div>
                    <div className="bg-white/10 backdrop-blur-xl rounded-lg p-4 border border-white/20">
                      <p className="text-white/70 text-sm mb-1">
                        {t.manageSubscription.orderId}
                      </p>
                      <p className="text-white font-mono text-xs break-all">
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1 bg-green-600/20 backdrop-blur-xl rounded-lg p-4 border border-green-400/30">
                        <p className="text-white/70 text-sm mb-1">
                          {t.manageSubscription.duration}
                        </p>
                        <p className="text-white font-bold">
                          {paymentNotification.details.months}{" "}
                          {paymentNotification.details.months > 1
                            ? t.manageSubscription.monthPlural
                            : t.manageSubscription.month}
                        </p>
                      </div>
                      <div className="flex-1 bg-green-600/20 backdrop-blur-xl rounded-lg p-4 border border-green-400/30">
                        <p className="text-white/70 text-sm mb-1">
                          {t.manageSubscription.amount}
                        </p>
                        <p className="text-green-400 font-bold">
                          ₹{paymentNotification.details.amount}
                        </p>
                      </div>
                    </div>
              ...
              {paymentNotification.type === "failed" &&
                paymentNotification.details && (
                  <>
                    {paymentNotification.details.orderId && (
                      <div className="bg-white/10 backdrop-blur-xl rounded-lg p-4 border border-white/20">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
          ...
          <div className="bg-green-600/80 backdrop-blur-xl text-white rounded-lg p-4 mb-6 flex items-center justify-between shadow-lg border border-green-400/50">
            ...
            <div className="text-right">
              ...
              <div className="text-sm opacity-90">
                {t.manageSubscription.daysRemaining}
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl shadow-2xl p-6 mb-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            ...
            >
              ...
              {loadingSubStatus
                ? t.manageSubscription.refreshing || "Refreshing..."
                : t.manageSubscription.refresh || "Refresh"}
            </button>
          </div>
          {loadingSubStatus ? (
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-white/70">
                    {t.manageSubscription.activeSubscription}
                  </p>
                </div>
                <div className="text-right">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="text-white font-semibold">
                    {t.manageSubscription.startDate}:
                  </span>
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-lg p-6 text-center border border-white/30">
              ...
              <p className="text-white mb-2">
                {t.manageSubscription.noActiveSubscription}
              </p>
              <p className="text-sm text-white/70">
                {t.manageSubscription.purchasePlanToStart}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl shadow-2xl p-6 mb-8">
          <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-2">
            ...
            {hasActivePlan
              ? t.manageSubscription.extendYourPlan
              : t.manageSubscription.purchasePremiumPlan}
          </h2>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-white/90">
                  {t.manageSubscription.unlimitedAccess}
                </p>
              </div>
              <div className="text-right">
                ...
                <div className="text-xs text-white/90 bg-white/20 px-2 py-1 rounded-full inline-block">
                  {PLAN.discount}% OFF
                </div>
              </div>
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex items-center gap-4">
                ...
                onChange={(e) =>
                  setSelectedMonths(
                    Math.max(
                      ...
                      Math.min(
                        PLAN.maxMonths,
                        parseInt(e.target.value) || PLAN.minMonths,
                      ),
                    ),
                  )
                }
              ...
              <div className="flex-1 text-right">
                <div className="text-sm text-white/70">
                  {t.manageSubscription.totalAmount}
                </div>
                <div className="text-3xl font-bold text-green-400">
                  ₹{calculateTotal()}
                </div>
          ...
          >
            {isProcessing
              ? t.manageSubscription.processing
              : hasActivePlan
                ? t.manageSubscription.extendPlan
                : t.manageSubscription.purchasePlan}
          </button>
        </div>
        ...
        <div
          ref={transactionRef}
          className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl shadow-2xl p-6 mb-8"
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            >
              ...
              {loadingHistory
                ? t.manageSubscription.refreshing
                : t.manageSubscription.refresh}
            </button>
          </div>
          {loadingHistory ? (
            <div className="text-center py-8">
                ...
                <thead className="bg-white/10 backdrop-blur-xl">
                  <tr>
                    <th className="px-4 py-3 text-left text-white font-semibold">
                      {t.manageSubscription.dateTime}
                    </th>
                    <th className="px-4 py-3 text-left text-white font-semibold">
                      {t.manageSubscription.orderId}
                    </th>
                    <th className="px-4 py-3 text-left text-white font-semibold">
                      {t.manageSubscription.transactionId}
                    </th>
                    <th className="px-4 py-3 text-left text-white font-semibold">
                      {t.manageSubscription.months}
                    </th>
                    <th className="px-4 py-3 text-left text-white font-semibold">
                      {t.manageSubscription.amount}
                    </th>
                    <th className="px-4 py-3 text-left text-white font-semibold">
                      {t.manageSubscription.paymentStatus}
                    </th>
                  </tr>
                </thead>
                  ...
                  {currentTransactions.map((transaction) => (
                    <tr key={transaction._id} className="hover:bg-white/5">
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold border ${getStatusBadgeClass(transaction.status)}`}
                        >
              ...
              {totalTransactionPages > 1 && (
                <div className="flex justify-center items-center gap-2 mt-6">
                  ...
                  <span className="text-white/80 text-sm font-medium px-4">
                    {t.manageSubscription.page} {transactionPage}{" "}
                    {t.manageSubscription.of} {totalTransactionPages}
                  </span>
                  <button
                    onClick={() =>
        ...
        <div
          ref={historyRef}
          className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl shadow-2xl p-6"
        >
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-4">
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
              <History className="w-6 h-6" />
          ...
          {loadingPlanHistory ? (
            ...
            <div className="relative">
                      ...
                      {sub.isActive && !sub.isExpired && (
                        <div className="mt-2 pt-2 border-t border-veag-green">
                          <span className="text-sm font-semibold text-white">
                            {sub.daysRemaining}{" "}
                            {t.manageSubscription.daysRemainingLower}
                          </span>
                        </div>
                      )}
                    </div>
              ...
              {totalPlanPages > 1 && (
                <div className="flex justify-center items-center gap-2 mt-8">
                  ...
                  <span className="text-white/80 text-sm font-medium px-4">
                    {t.manageSubscription.page} {historyPage}{" "}
                    {t.manageSubscription.of} {totalPlanPages}
                  </span>
                  <button
                    onClick={() =>
                      setHistoryPage((p) => Math.min(totalPlanPages, p + 1))
                    }
            ...
            <div className="text-center py-12">
              ...
              <p className="text-white mb-2">
                {t.manageSubscription.noPlanHistory}
              </p>
              <p className="text-sm text-white/70">
                {t.manageSubscription.purchaseFirstPlan}
              </p>
            </div>
          )}
        </div>

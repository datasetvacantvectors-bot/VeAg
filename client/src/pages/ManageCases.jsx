...
const ManageCases = ({ daysRemaining }) => {
  ...
  useEffect(() => {
    ...
    if (casesListRef.current) {
      const yOffset = -100;
      const y =
        casesListRef.current.getBoundingClientRect().top +
        window.scrollY +
        yOffset;
  ...
  const fetchCases = async (showRefreshLoader = false) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/cases/user/${currentUser.userId}`,
      );
      ...
      setError(
        err.response?.data?.error || "Failed to load cases. Please try again.",
      );
    } finally {
      if (showRefreshLoader) {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span
        className={`px-3 py-1 rounded-full text-xs font-semibold border backdrop-blur-xl ${statusClasses[status] || statusClasses.pending}`}
      >
  ...
  const filteredCases = cases
    .filter(
      (c) =>
        searchQuery.trim() === "" ||
        String(c.caseId).includes(searchQuery.trim()),
    )
    .sort((a, b) =>
      sortOrder === "latest"
    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-300 via-orange-200 to-yellow-100 relative overflow-hidden">
      ...
      {showSupport && (
        ...
        >
          ...
          <p className="text-white/90 mb-4">{t.manageCases.supportText}</p>
          <a
          ...
          <div className="bg-green-600/80 border border-green-400/50 backdrop-blur-xl rounded-lg p-4 mb-6 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-white font-semibold">
                {t.manageCases.activeSubscription}: {daysRemaining}{" "}
                {t.manageCases.daysRemaining}
              </span>
            </div>
          </div>
        )}
          ...
          filteredCases.length === 0 && (
            <div className="bg-black/30 backdrop-blur-2xl border border-white/40 rounded-2xl p-12 text-center shadow-2xl mb-6">
              <div className="flex flex-col items-center gap-4">
                ...
                <p className="text-white font-semibold text-lg">
                  No cases found for Case ID &quot;{searchQuery}&quot;
                </p>
              </div>
            </div>
          )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <p className="text-3xl font-bold text-blue-400">
                    {
                      filteredCases.filter((c) => c.status === "processing")
                        .length
                    }
                  </p>
                    ...
                    <div className="space-y-2 mb-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-white">
                          {caseItem.cropName
                            ? caseItem.cropName.charAt(0).toUpperCase() +
                              caseItem.cropName.slice(1)
                            : ""}
                        </span>
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex justify-center items-center gap-2 mb-8">
                ...
                <span className="text-white/80 text-sm font-medium px-4">
                  {t.manageCases.page} {currentPage} {t.manageCases.of}{" "}
                  {totalPages}
                </span>
                <button
                  onClick={() =>

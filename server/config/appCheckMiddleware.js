...
export const appCheckVerification = async (req, res, next) => {
  ...
  if (!appCheckToken) {
    return res.status(403).json({ 
      error: 'Forbidden. Missing Firebase App Check token. Request may not have originated from the official app.' 
    });
  }
  ...
  try {
    if (!admin.apps.length) {
      ...
      return res.status(500).json({ error: 'Server configuration error. App Check disabled.' });
    }

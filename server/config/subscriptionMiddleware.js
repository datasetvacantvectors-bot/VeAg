...
const subscriptionMiddleware = async (req, res, next) => {
  try {
    ...
    if (!req.user || !req.user.userId) {
      return res.status(401).json({ error: 'Unauthorized. User session missing or invalid.' });
    }
    ...
    if (!subscription) {
      return res.status(403).json({ 
        error: 'Forbidden. You do not have an active subscription to access this premium feature.',
    ...
    res.status(500).json({ error: 'Internal server error while verifying subscription status.' });
  }
};
